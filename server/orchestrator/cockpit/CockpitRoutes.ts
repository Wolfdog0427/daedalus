...
export function createCockpitRouter() {
  ...
  router.get(
    "/cockpit/nodes",
    (_req: Request, res: Response<CockpitNodeView[] | { error: string }>) => {
      try {
        const registry = getNodeMirrorRegistry();
        res.json(registry.toCockpitView());
      } catch (err: any) {
        console.error("[cockpit] /cockpit/nodes error:", err?.message);
        res.status(500).json({ error: err?.message ?? "Failed to fetch cockpit nodes" });
      }
    },
  );
  ...
  router.get(
    ...
    (req: Request, res: Response<CockpitNodeView | { error: string }>) => {
      try {
        ...
        console.error("[cockpit] /cockpit/nodes/:id error:", err?.message);
  ...
  router.get(
    ...
    (_req: Request, res: Response) => {
      try {
        ...
        console.error("[cockpit] /cockpit/summary error:", err?.message);
        res.status(500).json({ error: err?.message ?? "Failed to build cockpit summary" });
      }
    },
  );
